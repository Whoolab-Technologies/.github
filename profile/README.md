# Whoolab Technologies

We design and build scalable, production-ready web and mobile platforms for
businesses, startups, and enterprises — with a strong focus on workflow-driven,
data-centric, and domain-specific applications.

Our engineering approach emphasizes:

- Modular & scalable architecture
- Real-world business workflows
- Maintainable code and reusable components
- Practical production deployment experience

---

## 🏗️ Featured Open-Source Platform  
### Centralized Multi-Store Inventory Management System

A modular inventory platform designed for organizations operating across
multiple project sites or stores — such as construction companies,
infrastructure projects, or warehouse-based businesses.

The platform supports:

- Central + sub-store inventory model
- Store-to-store transfers
- MR (Material Request) workflows for engineers
- PR (Purchase Request) workflows for stock-out situations
- Transaction lifecycle statuses  
  (`processing`, `in-transfer`, `delivered`, etc.)
- Material consumption & accountability
- Return workflows (site → store → central)
- Auto-generated QR codes for all items
- QR scanning across operational workflows
- Role-based access for engineers & storekeepers

👉 Platform Overview  
https://github.com/Whoolab-Technologies/inventory-platform

---

## 🧩 Platform Repositories

- Shared Flutter Components  
  https://github.com/Whoolab-Technologies/inventory-flutter-common

- API Server (Laravel + PostgreSQL)  
  https://github.com/Whoolab-Technologies/inventory-api

- Admin Web Dashboard (Angular)  
  https://github.com/Whoolab-Technologies/inventory-admin-web

- Storekeeper Mobile App (Flutter)  
  https://github.com/Whoolab-Technologies/inventory-storekeeper-flutter

- Engineer Mobile App (Flutter)  
  https://github.com/Whoolab-Technologies/inventory-engineer-flutter

> This platform can be adapted to any domain with similar workflows —
> warehousing, site operations, asset/tools management, and more.

---

## 🧰 Technologies We Work With

- Flutter (mobile & shared UI frameworks)
- Angular (enterprise web dashboards)
- Laravel & PostgreSQL (API & workflows)
- Modular multi-repo system design

---

## 🤝 Customization, Deployment & Consulting

This project is available publicly for learning and evaluation purposes.

If your organization needs:

- Workflow customization
- Feature extensions
- Production deployment (cloud / on-prem)
- Long-term support & enhancements

we’d be happy to collaborate.

📩 Email: mail@whoolab.com  
🌐 Website: [(whoolab.com)](https://whoolab.com/)

---

## 🏷️ License

The platform is published for learning, demo, and non-commercial research use.

Commercial usage, resale, redistribution, or hosted service usage requires
a commercial license or service engagement with Whoolab Technologies.
